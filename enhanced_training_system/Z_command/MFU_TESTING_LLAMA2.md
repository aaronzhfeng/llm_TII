# MFU Testing Guide: LLaMA 2 1.36B on B200
...
# Expected: 45-55% MFU, ~180K tokens/sec
...
# Expected: 50-60% MFU, ~195K tokens/sec
...
# Expected: 55-65% MFU, ~210K tokens/sec
...
python << 'EOF'
import json, glob, os
log = sorted(glob.glob('out-llama-1.36b/run_*.json'), key=os.path.getmtime)[-1]
...
python << 'EOF'
import json, glob, os
logs = sorted(glob.glob('out-llama-1.36b/run_*.json'), key=os.path.getmtime, reverse=True)[:5]
...
# Terminal 3: Live MFU
watch -n 2 'tail -1 out-llama-1.36b/run_*.json | python3 -c "import sys,json; d=json.load(sys.stdin); it=d[\"training_iterations\"][-1]; print(f\"Iter {it[\"iter\"]} | MFU: {it[\"mfu\"]:.2f}% | Loss: {it[\"loss\"]:.4f}\")"'
```
...
|------|--------|-----|------------|
| **Baseline (2 GPU)** |
| 1.1 | Pure DDP, no compile | 40-50% | ~40K |
| 1.2 | + compile | 50-60% | ~50K |
| **Scaling (8 GPU)** |
| 2.1 | Pure DDP | 35-45% | ~140K |
| 2.2 | + compile | 45-55% | ~180K |
| **Production** |
| Tier 1 | Conservative | 45-55% | ~180K |
| Tier 2 | + DataLoader | 50-60% | ~195K |
| Tier 3 | + CUDA Graphs | 55-65% | ~210K |
